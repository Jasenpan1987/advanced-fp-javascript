...
// Don't forget to JSON.parse once it's pulled from the cache so you can _.get()
// the email
...
var cached = getCache("user");

console.log(runIO(cached).toString());
// runIO(ex6('user'))
var ex6 = compose(
  map(map(_.get("email"), JSON.parse)),
  getCache
);
